...
boards = ti.Vector.field(3, dtype=ti.f32, shape=12)
...
def confine_position_to_boundary(p):
    ...
    k1 = -boards[1][1] / 20
    k2 = boards[7][1] / 20
    if 0 < p[2] < 20:      # one board
        if p[1] <= (p[2] - 20) * k1 - particle_radius_in_world:
            y = p[1]
            z = p[2]
            p[2] = (z + k1 * y + 20 * k1 * k1) / (1 + k1 * k1) + epsilon * ti.random()
            p[1] = (p[2] - 20) * k1 + epsilon * ti.random()

    elif 20 <= p[2] < 40:  # another board
        if p[1] <= (p[2] - 20) * k2 - particle_radius_in_world:
            y = p[1]
            z = p[2]
            p[2] = (z + k2 * y + 20 * k2 * k2) / (1 + k2 * k2) - epsilon * ti.random()
            p[1] = (p[2] - 20) * k2 + epsilon * ti.random()
