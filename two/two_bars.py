...
def confine_position_to_boundary(p):
    ...
    # collision with bar
    k_value_left = kk[None][0]
    k_value_right = kk[None][1]
    if p[1] < k_value_left * (p[0] - 0.5 * boundary[0]) and p[0] <= 0.5 * boundary[0]:
        x = (k_value_left * p[1] + p[0] + 0.5 * k_value_left * k_value_left * boundary[0]) / (1 + k_value_left * k_value_left)
        y = k_value_left * (x - 0.5 * boundary[0])
        p[0] = x + epsilon * ti.random()
        p[1] = y + epsilon * ti.random()
    elif p[1] < k_value_right * (p[0] - 0.5 * boundary[0]) and p[0] > 0.5 * boundary[0]:
        x = (k_value_right * p[1] + p[0] + 0.5 * k_value_right * k_value_right * boundary[0]) / (1 + k_value_right * k_value_right)
        y = k_value_right * (x - 0.5 * boundary[0])
        p[0] = x - epsilon * ti.random()
        p[1] = y + epsilon * ti.random()
...
def draw(gui):
    ...
    gui.line(begin=(0.5, 0), end=(0, kk[None][0] * -0.5), color=0x000000, radius=2.5)

    gui.line(begin=(0.5, 0), end=(1, kk[None][1] * 0.5), color=0x000000, radius=2.5)
