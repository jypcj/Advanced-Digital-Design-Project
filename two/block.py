...
left_board = ti.Vector.field(2, float)
right_board = ti.Vector.field(2, float)

block = ti.Vector.field(2, float)
...
ti.root.place(left_board)
ti.root.place(right_board)
...
def confine_position_to_boundary(p):
    bmin = left_board[None][0]
    bmax = ti.Vector([right_board[None][0], boundary[1]
                      ]) - particle_radius_in_world
    for i in ti.static(range(dim)):
        if p[i] <= bmin and i == 0:
            p[i] = bmin + epsilon * ti.random()
        elif p[i] <= particle_radius_in_world and i == 1:
            p[i] = particle_radius_in_world + epsilon * ti.random()
        elif bmax[i] <= p[i]:
            p[i] = bmax[i] - epsilon * ti.random()

    # collision with block
    left_bottom = ti.Vector([block[None][0]])
    edge_length = ti.Vector([block[None][1]])
    if left_bottom[0] < p[0] < left_bottom[0] + edge_length[0] and 0 < p[1] < edge_length[0]:
        left = p[0] - left_bottom[0]
        right = edge_length[0] + left_bottom[0] - p[0]
        top = edge_length[0] - p[1]
        if left < right and left < top:
            p[0] = left_bottom[0] - epsilon * ti.random()
        elif right < left and right < top:
            p[0] = left_bottom[0] + edge_length[0] + epsilon * ti.random()
        elif top <= left and top <= right:
            p[1] = edge_length[0] + epsilon * ti.random()
...
def draw(gui):
    ...
    # draw left board
    gui.rect((0, 0), (left_board[None][0] / boundary[0], 1), radius=1.5, color=0x000000)

    # draw right board
    gui.rect((0, 0), (right_board[None][0] / boundary[0], 1), radius=1.5, color=0x000000)

    # draw the block
    left_bottom = [block[None][0], 0]
    right_top = [block[None][0] + block[None][1], block[None][1]]
    gui.rect((left_bottom[0] * screen_to_world_ratio / screen_res[0],
              left_bottom[1] * screen_to_world_ratio / screen_res[1]),
             (right_top[0] * screen_to_world_ratio / screen_res[0],
              right_top[1] * screen_to_world_ratio/ screen_res[1]), radius=2.5, color=0x000000)
...
def init_particles():
    ...
    right_board[None] = ti.Vector([boundary[0] - epsilon, -0.0])
    left_board[None] = ti.Vector([0, 0])

    block[None] = ti.Vector([20, 20])
